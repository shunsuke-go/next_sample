...
import { Flex, Box, FormControl, FormLabel, Input, FormErrorMessage, Button, Radio, RadioGroup } from '@chakra-ui/react'
...
export const TaskForm: React.FC<Props> = ({ onSubmit, task, isUpdate }) => {
  ...
  return (
    <Flex flexDirection='column' alignItems='center' mt={5}>
      <form onSubmit={handleSubmit((form) => onSubmit(form, task?.id))}>
        <Flex flexDirection='column' alignItems='center' mt={5}>
          ...
          <FormControl isInvalid={!!errors.description} mt={5}>
            ...
            <Input {...register('description')} defaultValue={task?.description} />
            <FormErrorMessage>{errors.description?.message}</FormErrorMessage>
          </FormControl>
          {isUpdate &&
            <FormControl isInvalid={!!errors.description} mt={5}>
              <FormLabel htmlFor='isDone' fontSize={'12px'}>isDone?</FormLabel>
              <Controller
                name='isDone'
                control={control}
