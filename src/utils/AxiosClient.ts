...
export class AxiosClient {
  ...
  public async get<T> ({ url, config }: { url: string, config?: AxiosRequestConfig }) {
    const res = await this.axiosInstance.get<T>(url, config)
    return res.data
  }

  public async post<T> ({ url, data, config }: { url: string, data: any, config?: AxiosRequestConfig }) {
    const res = await this.axiosInstance.post<T>(url, data, config)
    return res.data
  }

  public async delete<T> ({ url, config }: { url: string, config?: AxiosRequestConfig }) {
    const res = await this.axiosInstance.delete<T>(url, config)
    return res.data
  }

  public async patch<T extends void> ({ url, data, config }: { url: string, data: any, config?: AxiosRequestConfig }) {
